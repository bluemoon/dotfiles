...
;;;###autoload
(defun grizzl-custom-completing-read (prompt index)
      ...
      (lambda ()
        ...
        (lexical-let*
            ((hookfun (lambda ()
                        ...
                        ;; (when (not (= (grizzl-selected-result index) nil))
                        ;;   ;; (view-tag-other-window (grizzl-selected-result index)))
                         ;; (condition-case nil
                         ;;     (display-buffer
                         ;;      (find-tag-noselect 
                         ;;       (grizzl-selected-result index)) t)
...
(defun grizzl-selected-result (index)
  ...
  ;; (message (grizzl-current-selection))
...
(defun grizzl-move-selection (delta)
  ...
  ;; (message (prin1 *grizzl-current-result*))
